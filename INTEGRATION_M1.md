# Intégration brique L6.1 ↔ schéma PostgreSQL (M1)

Ce document met en correspondance ce que **le client de lecture (L6.1)** fournit
et ce que **le backend / schéma PostgreSQL (M1)** attend. Il sert de base de
discussion entre M4 (client) et M1 (backend + base).

Il s'appuie sur le schéma réel observé dans `soc-agentique/backend`
(`models/models.py` et `app/schemas/alertes.py`).

## Principe

Le client est en **lecture seule sur OpenSearch**. Il ne touche pas PostgreSQL.
Chaque objet typé expose `to_dict()` produisant les **valeurs lisibles** (noms,
identifiants métier, sévérité déjà convertie vers l'enum M1) plus le **document
OpenSearch complet** sous la clé `donnees_brutes`.

C'est le **backend (M1)** qui insère : il résout les références (UUID d'agent, de
règle) et remplit la table.

## Schéma réel de M1 (table `alertes`)
...
Ce que fournit `Alert.to_dict()` → ce que M1 en fait :

| Champ `AlerteCreate` | Fourni par la brique                 | Traitement côté backend M1 |
|----------------------|--------------------------------------|----------------------------|
...
> La brique **convertit déjà la sévérité** vers l'enum M1 via `severite_m1()`
> (exposé sous la clé `severite` de `to_dict()`). M1 n'a donc que les **UUID** à
> résoudre.

Table de conversion appliquée par la brique :

| Source                | Valeur              | → enum M1  |
|-----------------------|---------------------|------------|
...
## Points ouverts à valider avec M1
...
1. **Le modèle `Alerte` est conçu pour Wazuh/Sigma, pas pour les 4 sources.**
   ...
   **obligatoires**. Or :
   ...
   → **Comment M1 veut-il stocker findings et alertes de détecteur ?** Même table
...
- Les 3 sources d'alertes exposent `severite` déjà convertie vers l'enum M1.
...
et `inserer_alerte` sont à implémenter par M1 (elles accèdent à la base ; la
brique, elle, ne fait que lire OpenSearch). Voir aussi
`exemple_integration_m1.py`.

```python
from soc_reader_client import SOCReader
# from app.schemas.alertes import AlerteCreate   # côté backend M1
